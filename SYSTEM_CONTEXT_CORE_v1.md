...
- **backend_permissions:** manage_users, manage_sections, manage_products, manage_topups, view_sales, create_orders, edit_orders, delete_orders, view_orders, view_fulfillments, manage_fulfillments, view_refunds, process_refunds, view_activities, manage_settlements, view_dashboard, manage_bugs, update_product_prices.
...
- **Dashboard access:** `/dashboard` additionally requires `can:view_dashboard` (not just backend middleware). Migration `2026_04_06_120000_add_view_dashboard_permission` seeds this permission for admin and supervisor.
- **Pricing maintenance access:** `/product-entry-prices` requires `can:update_product_prices` and is intended for controlled entry-price operations.
...
- **RTL / locale:** `lang` en | ar; session locale; RTL when `app()->isLocale('ar')`. Header sets `dir` and `lang`. Language switch route persists both session locale and user preference (`locale_manually_set=true`) for authenticated users.
...
- **Locale sync order after login/2FA:** (1) session locale when set, (2) saved user locale when manually locked, (3) `Accept-Language` fallback via `App\Support\SupportedLocale` (`ALLOWED=['en','ar']`) and primary locale order from config.
...
- **Custom amount coverage:** Tests such as `BuyNowModalTest`, `CheckoutFlowTest`, `FulfillmentActionsTest`, `OrdersPageTest`, `PriceCalculatorTest`, `ProductModelTest`, `ProductsPageTest`, `UserProductPriceFeatureTest` exercise custom-amount products end-to-end where applicable. Buy-now custom amount quote API is implemented via `BuyNowCustomAmountQuoteController` + `App\Actions\Pricing\QuoteBuyNowCustomAmountLine` and exposed at `POST /api/pricing/buy-now-custom-amount-quote`.
...
- **Auth + verified:** `/profile`, `/profile/edit`, `/wallet`, `/loyalty`, `/orders`, `/orders/{order_number}`, `/notifications`, `/topup-proofs/{proof}`, `/bug-attachments/{attachment}`, `POST /api/pricing/buy-now-custom-amount-quote`.
- **Backend (auth + verified + backend):** `/dashboard` (**also `can:view_dashboard`**), `/categories`, `/packages`, `/products`, `/product-entry-prices` (**`can:update_product_prices`**), `/pricing-rules`, `/loyalty-tiers`, `/admin/orders`, `/admin/orders/{order}`, `/admin/activities`, `/admin/system-events`, `/admin/users`, `/admin/users/{user}`, `/admin/users/{user}/audit`, `/fulfillments`, `/refunds`, `/topups`, `/customer-funds`, `/settlements`, `/admin/notifications`, `/admin/bugs`, `/admin/bugs/{bug}`. **Admin-only:** `/admin/website-settings`.
